...
import com.mordenkainen.equivalentenergistics.blocks.base.block.*;
import com.mordenkainen.equivalentenergistics.blocks.condenser.tiles.*;
...
public class BlockEMCCondenser extends BlockMultiContainerBase implements IConfigurable, ILayeredBlock{
    ...
    @Override public boolean canRenderInPass(final int pass){return pass==1;}
    @Override public boolean renderAsNormalBlock(){return false;}
    @Override public int getRenderType(){return EquivalentEnergistics.proxy.layeredRenderer;}
    @Override public TileEntity createNewTileEntity(final World world,final int meta){
        switch(meta){
            case 0:
    ...
    @SideOnly(Side.CLIENT)@Override public IIcon getIcon(final int side,final int meta){return TextureEnum.EMCCONDENSER.getTexture(meta);}
    ...
    @Override public int getComparatorInputOverride(final World world,final int x,final int y,final int z,final int meta){
        if(meta==0)return 0;
        final TileEMCCondenserAdv tile=CommonUtils.getTE(TileEMCCondenserAdv.class,world,x,y,z);
        switch(Objects.requireNonNull(tile).getState()){
            case ACTIVE:
                return 2;
            case IDLE:
                return 1;
    ...
    @Override public boolean canConnectRedstone(final IBlockAccess world,final int x,final int y,final int z,final int side){return world.getBlockMetadata(x,y,z)!=0;}
    @Override public int numLayers(final Block block,final int meta){
        return 0;
    }
    @Override public int numLayers(final IBlockAccess world,final Block block,final int x,final int y,final int z,final int meta){return 2;}
    @Override public IIcon getLayer(final Block block,final int side,final int meta,final int layer){
        return null;
    }
    @Override public IIcon getLayer(final IBlockAccess world,final Block block,final int x,final int y,final int z,final int side,final int meta,final int layer){
        final TileEMCCondenserBase tileCondenser=CommonUtils.getTE(TileEMCCondenserBase.class,world,x,y,z);
        if(tileCondenser!=null){
            if(layer==1){
                if(tileCondenser.getState().isError())return TextureEnum.EMCCONDENSEROVL.getTexture(3);
                else if(tileCondenser.isActive())return TextureEnum.EMCCONDENSEROVL.getTexture(2);
            }
            else if(tileCondenser instanceof TileEMCCondenserExt){
                if(((TileEMCCondenserExt)tileCondenser).getSide(side)==SideSetting.INPUT)return TextureEnum.EMCCONDENSEROVL.getTexture();
                else if(((TileEMCCondenserExt)tileCondenser).getSide(side)==SideSetting.OUTPUT)return TextureEnum.EMCCONDENSEROVL.getTexture(1);
            }
        }
        return null;
    }
    @Override public void loadConfig(final Configuration config){
