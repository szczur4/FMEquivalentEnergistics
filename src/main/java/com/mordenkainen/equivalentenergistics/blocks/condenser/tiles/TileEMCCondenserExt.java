...
import com.mordenkainen.equivalentenergistics.util.inventory.*;
import java.util.*;
...
public class TileEMCCondenserExt extends TileEMCCondenserAdv{
    ...
    private final Map<ForgeDirection,SideSetting> sides=new HashMap<>();
    public TileEMCCondenserExt(){this(new ItemStack(Item.getItemFromBlock(BlockEnum.EMCCONDENSER.getBlock()),1,2));}
    public TileEMCCondenserExt(final ItemStack repItem){
        super(repItem);
        internalInventory=new InternalInventory("EMCCondenserInventory",4,64);
        for(final ForgeDirection side: ForgeDirection.VALID_DIRECTIONS)sides.put(side,SideSetting.NONE);
    }
    @Override
    ...
    protected void importItems(){
        ...
        for(final ForgeDirection side: sides.keySet()){
            if(sides.get(side)!=SideSetting.INPUT)continue;
            ...
            if(sourceInv!=null)numItems-=InvUtils.extractWithCount(side.getOpposite(),sourceInv,getInventory(),numItems);
            if(numItems<=0)break;
        }
    }
    @Override protected double getEMCPerTick(){return BlockEMCCondenser.emcPerTick*100;}
    @Override protected ItemStack ejectItem(final ItemStack stack){
        int numItems=itemsToTransfer();
        for(final ForgeDirection side: sides.keySet()){
            if(sides.get(side)!=SideSetting.OUTPUT)continue;
        ...
        if(stack.stackSize>0)return super.ejectItem(stack);
        else return null;
    }
    @Override protected void getPacketData(final NBTTagCompound nbttagcompound){
        ...
        for(final ForgeDirection side: sides.keySet())list.setInteger(side.name(),sides.get(side).ordinal());
    ...
    @Override public void readFromNBT(final NBTTagCompound data){
        ...
        if(list!=null)sides.replaceAll((s,v)->SideSetting.values()[list.getInteger(s.name())]);
    }
    ...
    public SideSetting getSide(final int side){return sides.get(ForgeDirection.getOrientation(side));}
    protected int itemsToTransfer(){return 16;}
