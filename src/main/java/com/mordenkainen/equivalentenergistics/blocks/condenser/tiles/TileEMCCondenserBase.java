...
import com.mordenkainen.equivalentenergistics.blocks.condenser.*;
import com.mordenkainen.equivalentenergistics.integration.Integration;
import com.mordenkainen.equivalentenergistics.integration.ae2.cache.storage.IEMCStorageGrid;
import com.mordenkainen.equivalentenergistics.integration.ae2.grid.*;
...
public abstract class TileEMCCondenserBase extends TileAEInv implements IGridTickable{
    ...
    @Override public TickRateModulation tickingRequest(final IGridNode node,final int ticksSinceLast){
        ...
        CondenserState newState;
        if(!isActive()||getInventory().isEmpty())updateState(CondenserState.IDLE);
        else{
            newState=processInv();
    ...
    public CondenserState getState(){return state;}
    ...
    protected double processItems(final int slot,final double remainingEMC,final boolean usePower){
        ...
        try{
            ...
            if(itemEMC>availEMC)return -1;
            if(itemEMC>remainingEMC)return remainingEMC;
            int maxToDo=Math.min(stack.stackSize,Math.min((int)(availEMC/itemEMC),(int)(remainingEMC/itemEMC)));
            if(usePower)maxToDo=Math.min(getMaxItemsForPower(maxToDo,itemEMC),maxToDo);
            if(maxToDo<=0)return -3;
            final double toStore=itemEMC*maxToDo;
            if(usePower)GridUtils.extractAEPower(getProxy(),toStore*BlockEMCCondenser.activePower,Actionable.MODULATE,PowerMultiplier.CONFIG);
    ...
    protected double processStorage(final int slot,final double remainingEMC){
        ...
        try{
            ...
            if(Integration.emcHandler.getStoredEMC(stack)<=0){
                ...
                if(getInventory().getStackInSlot(slot)!=null)return -2;
            }
            return remainingEMC-toStore;
        }catch(GridAccessException e){
    ...
    protected CondenserState processInv(){
        ...
        for(int slot=0;slot<4&&remainingEMC>0;slot++){
            ...
            if(Integration.emcHandler.isEMCStorage(stack))remainingEMC=processStorage(slot,remainingEMC);
            else if(Integration.emcHandler.hasEMC(stack))remainingEMC=processItems(slot,remainingEMC,!ItemEnum.isCrystal(stack));
            else{
                getInventory().setInventorySlotContents(slot,ejectItem(stack));
                if(getInventory().getStackInSlot(slot)!=null)remainingEMC=-2;
            }
        }
    ...
    protected ItemStack ejectItem(final ItemStack stack){return GridUtils.injectItemsForPower(getProxy(),stack,mySource);}
    protected class CondenserInventory extends InternalInventory{
        CondenserInventory(){super("EMCCondenserInventory",4,64);}
        @Override public boolean isItemValidForSlot(final int slotId,final ItemStack itemStack){return Integration.emcHandler.isEMCStorage(itemStack)||Integration.emcHandler.hasEMC(itemStack)&&Integration.emcHandler.getSingleEnergyValue(itemStack)<=getEMCPerTick();}
    }
}
