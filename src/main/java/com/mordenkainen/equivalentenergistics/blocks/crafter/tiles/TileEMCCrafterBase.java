...
public abstract class TileEMCCrafterBase extends TileAEBase implements ICraftingProvider, IGridTickable, IWailaNBTProvider, IDropItems, ICraftingMonitor, ITransProvider{
    ...
    @Override public NBTTagCompound getWailaTag(final NBTTagCompound tag){
        if(currentEMC>0)tag.setDouble(EMC_TAG,currentEMC);
        if(transmutationItem!=null)tag.setString(OWNER_TAG,Integration.emcHandler.getTomeOwner(transmutationItem));
        return tag;
    }
    @Override public TickingRequest getTickingRequest(final IGridNode node){return new TickingRequest(1,20,false,true);}
    @Override public TickRateModulation tickingRequest(final IGridNode node,final int ticksSinceLast){
        if(refreshNetworkState())markForUpdate();
        if(!isActive())return TickRateModulation.IDLE;
        injectEMC();
    ...
    @Override public void readFromNBT(final NBTTagCompound data){
    ...
    @Override public void writeToNBT(final NBTTagCompound data){
        super.writeToNBT(data);
        if(currentEMC>0)data.setDouble(EMC_TAG,currentEMC);
        if(transmutationItem!=null)data.setTag(TOME_TAG,transmutationItem.writeToNBT(new NBTTagCompound()));
        manager.writeToNBT(data);
    }
    @Override public boolean pushPattern(final ICraftingPatternDetails patternDetails,final InventoryCrafting table){
    ...
    @Override public boolean isBusy(){return manager.isBusy();}
    @Override public void provideCrafting(final ICraftingProviderHelper craftingTracker){GridUtils.addPatterns(getProxy(),this,craftingTracker);}
    public ItemStack getCurrentTome(){return transmutationItem;}
    public void setCurrentTome(final ItemStack heldItem){
        transmutationItem=heldItem;
        GridUtils.updatePatterns(getProxy());
        markForUpdate();
    }
    public boolean canPlayerInteract(final EntityPlayer player){return checkPermissions(player)&&!manager.isCrafting();}
    private void injectEMC(){currentEMC-=GridUtils.injectEMC(getProxy(),currentEMC,Actionable.MODULATE);}
    @Override public void getDrops(final World world,final int x,final int y,final int z,final List<ItemStack> drops){if(doDrops&&transmutationItem!=null)drops.add(transmutationItem);}
    @Override public void disableDrops(){doDrops=false;}
    @Override protected void getPacketData(final NBTTagCompound nbttagcompound){
        ...
        if(manager.isCrafting()){
            ...
            for(int i=0;i<displayStacks.size();i++)if(displayStacks.get(i)!=null)displayTags.setTag(STACK_TAG+i,displayStacks.get(i).writeToNBT(new NBTTagCompound()));
            nbttagcompound.setTag(DISPLAY_TAG,displayTags);
        }
        if(transmutationItem!=null)nbttagcompound.setTag(TOME_TAG,transmutationItem.writeToNBT(new NBTTagCompound()));
    }
    @Override protected boolean readPacketData(final NBTTagCompound nbttagcompound){
        ...
        if(nbttagcompound.hasKey(DISPLAY_TAG)){
            ...
            for(int i=0;i<maxJobs;i++){
                if(invList.hasKey(STACK_TAG+i))displayStacks.add(ItemStack.loadItemStackFromNBT(invList.getCompoundTag(STACK_TAG+i)));
                else displayStacks.add(null);
            }
        }
    ...
    @Override public String getPlayerUUID(){return transmutationItem==null?null:Integration.emcHandler.getTomeUUID(transmutationItem).toString();}
    @Override public List<ItemStack> getTransmutations(){return transmutationItem==null?new ArrayList<>():Integration.emcHandler.getTransmutations(this);}
    @Override public void craftingFinished(final ItemStack outputStack){
        displayStacks=manager.getCurrentJobs();
        markForUpdate();
    }
