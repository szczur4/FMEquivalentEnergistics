...
public class TileEMCCrafterRenderer extends TileEntitySpecialRenderer{
    ...
    private static final ResourceLocation[]MODELTEXTURES={new ResourceLocation(Reference.MOD_ID+":textures/models/EMCCrafter.png"),new ResourceLocation(Reference.MOD_ID+":textures/models/EMCCrafterAdv.png"),new ResourceLocation(Reference.MOD_ID+":textures/models/EMCCrafterExt.png"),new ResourceLocation(Reference.MOD_ID+":textures/models/EMCCrafterUlt.png")};
    @Override public void renderTileEntityAt(final TileEntity tile,final double x,final double y,final double z,final float partialTicks){
        if(!(tile instanceof TileEMCCrafterBase crafter))return;
        renderCrafter(x,y,z,crafter.getBlockMetadata());
        if(crafter.getWorldObj()!=null){
            if(crafter.isActive())renderLights(crafter,x,y,z);
    ...
    private void renderConnectors(final TileEMCCrafterBase tile,final double x,final double y,final double z){
        ...
        for(final ForgeDirection side: ForgeDirection.VALID_DIRECTIONS)if(isCableConnected(tile.getWorldObj(),tile.xCoord,tile.yCoord,tile.zCoord,side))MODEL.renderConnector(side);
        GL11.glPopMatrix();
    }
    ...
    private boolean isCableConnected(final IBlockAccess world,final int x,final int y,final int z,final ForgeDirection side){
        ...
        if(tileYPos<0||tileYPos>256)return false;
        final TileEntity tile=world.getTileEntity(x+side.offsetX,tileYPos,z+side.offsetZ);
        if(!(tile instanceof IGridHost&&tile instanceof IPartHost host))return false;
        final IPart part=host.getPart(ForgeDirection.UNKNOWN);
        if(part instanceof IPartCable cable)return cable.isConnected(side.getOpposite());
        return false;
    }
}
