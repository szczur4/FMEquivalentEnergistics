...
public class CraftingManager{
    ...
    public boolean isBusy(){
        for(int i=0;i<maxJobs;i++)if(jobs[i]==null)return false;
        return true;
    }
    public boolean isCrafting(){
        for(int i=0;i<maxJobs;i++)if(jobs[i]!=null)return true;
        return false;
    }
    public boolean addJob(final ItemStack outputStack,final double emc,final double powerPerEMC){
        if(!isBusy())for(int i=0;i<maxJobs;i++)if(jobs[i]==null){
    ...
    public boolean craftingTick(){
        for(int i=0;i<maxJobs;i++)if(jobs[i]!=null){
            if(!jobs[i].craftingTick())return false;
