...
import com.mordenkainen.equivalentenergistics.blocks.crafter.tiles.*;
...
public class BlockEMCCrafter extends BlockMultiContainerBase implements IConfigurable{
    ...
    @Override public int getRenderType(){return EquivalentEnergistics.proxy.crafterRenderer;}
    @SideOnly(Side.CLIENT)@Override public IIcon getIcon(final int side,final int meta){return TextureEnum.EMCCONDENSER.getTexture();}
    @Override public final boolean onBlockActivated(final World world,final int x,final int y,final int z,final EntityPlayer player,final int side,final float hitX,final float hitY,final float hitZ){
        final TileEMCCrafterBase tileCrafter=CommonUtils.getTE(TileEMCCrafterBase.class,world,x,y,z);
        if(tileCrafter==null||!tileCrafter.canPlayerInteract(player))return false;
        ...
        if(Integration.emcHandler.isValidTome(player.getHeldItem())&&existingTome==null){
            ...
            if(!world.isRemote)CommonUtils.spawnEntItem(world,x,y,z,existingTome);
            return true;
        }
        return false;
    }
