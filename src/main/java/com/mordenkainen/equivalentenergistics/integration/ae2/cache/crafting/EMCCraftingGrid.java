...
public class EMCCraftingGrid implements IEMCCraftingGrid{
    private static final Equivalence<ItemStack> eq=new CompItemStack();
    private static final Map<Equivalence.Wrapper<ItemStack>,EMCCraftingPattern> patternList=new HashMap<Equivalence.Wrapper<ItemStack>,EMCCraftingPattern>();
    private static int patternVer;
    private static final Map<EMCCraftingGrid,Boolean> craftingGrids=new WeakHashMap<EMCCraftingGrid,Boolean>();
    ...
    private void postKnowledgeEvent(final UUID playerUUID){
        if(patternProviders.containsValue(playerUUID.toString())){
            lastPatternVer=-1;
        }
    }
    @Override
